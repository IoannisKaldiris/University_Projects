...
int main(int argc, char *argv[]) {
    double a = 0.0;
    double b = 1.0;
    unsigned long n = 24e7; // Default number of iterations
    ...
    // Δημιουργία σημαφόρου για την προσταρία της shared memory

    sem_t *semaphore = sem_open("/shared_memory_protection", O_CREAT | O_EXCL, 0644, 1);
    ...
    for (int worker = 0; worker < total_processes; worker++) {
        ...
        if (pid < 0) {
            ...
            munmap(shared_memory, memory_block);
            sem_close(semaphore);
            sem_unlink("/shared_memory_protection");
            exit(1);
        }
        ...
        if (pid == 0) { // Κώδικας που εκτελείται από το παιδί
            ...
            // Critical section: ανανέωση της shared memory

            sem_wait(semaphore);  // Κλειδώνουμε τον semaphore

            shared_memory[worker] = local_sum; // Εγγραφή του μερικού αποτελέσματος

            sem_post(semaphore);  // Ξεκλείδωμα semaphore
    ...
    // Εκκαθάριση semaphore και shared memory
