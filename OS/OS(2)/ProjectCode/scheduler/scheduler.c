...
void file_print(struct Process* process, double elapsed_seconds){
	FILE *fp = fopen("results.txt", "a");
    if (fp == NULL) {
        perror("Unable to open file");
        return ;
    }
	fprintf(fp,"PID %d - CMD: %s\n", process->pid, process-> executable_path);

	fprintf(fp,"	Elapsed time = %.2f secs \n", elapsed_seconds);
	fprintf(fp,"	Workload time = %.2f secs \n", process->workload_time);
    fclose(fp);
    return ;
}
...
void print_process(struct Process* process){
	...
	//file_print(process,elapsed_seconds);
...
// Function to calculate the elapsed time from time zero
...
void RR(int quantum){
	...
	while(next_node != NULL){
		...
		// Handle each process based on state
		...
		if(current_node->process->state == STOPPED){	// If child stoped add to queue
