...
double f(double *x, int n)
{
	...
	// This thing is really really important, Since funevals is a global varaible and is increment by mutiple thread
...
int main(int argc, char *argv[])
{
	...
	// Set the number of thread used in the parallel region 
	if (argc > 1) {
	...
	// Pragam clause for parallel 
	// #pragma omp parallel for -> is used to parallelize the for loop
	// firstprivate -> is to keep the initial value of the variable in the parallel region, but a local copy is created for each thread
	// private -> is to keep the variable private to each thread, so that each thread can have it's own copy of the variable
	// shared -> is to keep the variable shared among all the threads, so that all the threads can access the variable
	#pragma omp parallel for firstprivate(startpt, endpt) private(trial, i, nt, nf, fx) shared(best_trial, best_nt, best_nf, best_fx, best_pt)
	for (trial = 0; trial < ntrials; trial++) {
		// it will then not produce the same as serial program 
		...
		if (fx < best_fx) {
			...
			#pragma omp critical
			{
				// the recheck ensure that the entered thread actually do have best result 
				if(fx < best_fx) {
					best_trial = trial;
					best_nt = nt;
					best_nf = nf;
